	...
	$CI->email->from('lapas3329@gmail.com', 'LAPAS CABANG RUTAN');
	    ...
	    function up($imges,$url){
	    	...
	    	if ( ! $CI->upload->do_upload($imges)){
	    		...
	    		$image = $CI->upload->data('full_path');
	    		return  $image;
	    	}
	    }
	    ...
	    function option_tahun($mulai,$selesai,$mode){
	    	if ($mode == 'up') {
	    		$option='';
...
// kostanstanta nama class
	    function _class($no){
	    	$name_class = ['Admin','Admin_ajax'];
	    	return $name_class[$no];
	    }
// fungsi cek sistem operasi 
...
//         $platform = 'windows';
//     }elseif (preg_match('/linux/android', $u_agent)) {
//         $platform = 'andro';
//     }
